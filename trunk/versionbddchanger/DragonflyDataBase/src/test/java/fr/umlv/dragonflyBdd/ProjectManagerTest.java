...
public class ProjectManagerTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		am.createAccount(userToAdd, nickname+"usertoAdd", password);
		am.activateAccount(userToAdd);
		
		am.createAccount(user2, nickname+"user2", password);
	...
	/**
	 * Test method for {@link fr.umlv.dragonflyBdd.ProjectManager#removeUserFromProject(java.lang.String, java.lang.String)}.
	 * @throws DragonflyBddException 
	 */
	public void testRemoveUserFromProject() throws DragonflyBddException {
		
		//Remove an existing account non yet removed from Project
		assertTrue(pm.removeUserFromProject(projectName, userToAdd));
		
		//Try to remove an existing account allready removed from Project
		assertFalse(pm.removeUserFromProject(projectName, userToAdd));
		
		//Try to remove a non existing account from Project
		assertFalse(pm.removeUserFromProject(projectName, "nonExistingAccount@test.org"));
