...
public class AccountManager {
     ...
     * @return 0 - Account created ; 1 - Account with this email allready exist ; 2 - Error during Account creation ; 3 - NickName already exist
    ...
    public int createAccount(String email, String nickname, String password) throws DragonflyBddException{
        ...
        try{
            ...
            Criteria criteria = session.createCriteria(User.class).add(Restrictions.eq("nickname",nickname));
        ...
        try {
            ...
            for(Roles role : roles ){
                ...
                if(role.getRole().endsWith("admin")){
                    ...
                    Set<User> set = (Set) PM_INSTANCE.getProjectUsers(project);
                    ...
                    //Sinon on affecte un nouvel utilisateur comme chef de projet (premier de la list)
                        ...
                        PM_INSTANCE.removeUserFromProject(project,email);                        
                        //Update projet
                        session.update(p);
                    }**/
    ...
    public boolean changeNickmane(String email, String newNickName) throws DragonflyBddException{
        ...
        try {
            session = DragonFlyDBManager.openSession();
            tr = session.beginTransaction();
            Criteria criteria = session.createCriteria(User.class).add(Restrictions.eq("nickname",newNickName));
            if( criteria.uniqueResult() != null)
                return false;
